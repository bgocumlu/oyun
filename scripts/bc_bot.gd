...
func _process(_delta: float) -> void:
	...
	if target:
		...
		if $AnimatedSprite2D.flip_h:
			$AnimatedSprite2D.position = Vector2(3, -3)
			hitbox.position = Vector2(-23, 3)
		else:
			$AnimatedSprite2D.position = Vector2(-3, -3)
			hitbox.position = Vector2(23, 3)
	elif velocity.x:
...
func _follow_state(_delta: float):
	...
	if length < 45 and direction.y > -12 and direction.y < 25 and is_instance_valid(target) and healthbar.health > 0:
...
func _attack_state(_delta: float):
	if entry:
		...
		var length: float = direction.length()
		entry = true
		if length > 45 or direction.y < -12 or direction.y > 25:
			change_state(States.Follow)
